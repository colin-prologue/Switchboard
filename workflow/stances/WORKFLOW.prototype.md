...
- **ESCALATE** — the diff contains something on the escalation list, a finding
  ...
  **If — and only if — you are escalating because the review bot has not
  ...
  ```
  gh pr comment <pr> --repo {{REPO}} --body "<!-- switchboard:escalated-pending-review sha=<head-sha> bot={{REVIEW_BOT}} -->
  Escalated to the human gate waiting on a cross-model review of <head-sha> by {{REVIEW_BOT}}. If that review lands clean, Switchboard returns this issue to review automatically.
