...
- AgDRs: ADR-001 (guard logic in tested `hooks/` package, not `sb/`), ADR-002 (`.switchboard/` upward discovery from worktree cwd), ADR-003 (guard arms on 2nd trip OR exhausted nudge budget) — all `pending-review` for the next gate
