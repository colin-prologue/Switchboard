...
- **M0 A-continuation IMPLEMENTED** (2026-06-21, 181 tests, branch `plan/sb-continuation`): research-handoff chain. Engine `spawn_research` was already built; this added the `paused_for_research` result outcome + `research` block (result schema 0.2.0), `file-result`→`spawn_research` delegation (ADR-005), the `sb result` read verb feeding research findings to the continuation (ADR-006), prompts, and a full-chain integration test. ADR-005/006 pending-review
- Next: A-planner, then C (HDR-010 escalation + ADR-004 learning loop), then the D exit bar (see ROADMAP). Pending-review AgDRs: ADR-005/006 (this round); ADR-001/002/003/004 already approved at the PR #2 gate
- Engine surface (Plan 1+2+3-A + A-continuation): `sb init|seed|claim|file-result|result|release|block|spawn|requeue-stale|query|heartbeat|status|brief|stamp|notify`; exit codes 0 ok / 2 held / 3 nothing-to-claim (also: `sb result <id>` exits 3 = no result yet). B adds no verbs — it is hooks + a monitor wrapping existing read verbs. A-continuation adds `result` (read verb) + the `paused_for_research` result outcome (engine spawns research + re-enqueues the parent as a continuation)
