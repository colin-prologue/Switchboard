...
- **Next: A-planner** — `sb seed --goal` + planner prompt protocol (planning is a task type). Carries an open design question (how `sb seed --goal` represents a plan-task; see ROADMAP A-planner bullet). Then C (HDR-010 escalation + ADR-004 learning loop), then the D exit bar (see ROADMAP)
- All AgDRs approved: ADR-001/002/003/004 (PR #2 gate, 2026-06-19), ADR-005/006 (PR #3 merge gate, 2026-06-21). No pending-review AgDRs
