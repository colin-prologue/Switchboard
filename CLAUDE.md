...
- Engine surface (Plan 1+2+3-A + A-continuation + A-planner): `sb init|seed|claim|file-result|result|release|block|spawn|requeue-stale|query|heartbeat|status|brief|stamp|notify`; exit codes 0 ok / 2 held / 3 nothing-to-claim (also: `sb result <id>` exits 3 = no result yet). `sb seed` now takes `--plan <path>` (expand a plan) XOR `--goal "<goal>"` XOR `--goal-file <path>` (`-` = stdin; for multi-line/voice-distilled briefs) — the latter two enqueue one planner task (`--tier` default opus; goal stored verbatim, no length cap). B adds no verbs. A-continuation adds `result` (read verb) + the `paused_for_research` result outcome (engine spawns research + re-enqueues the parent as a continuation)
