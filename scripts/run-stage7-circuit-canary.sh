#!/usr/bin/env bash
# Run one reviewed Stage 7 mixed-canary circuit checkpoint from macOS Terminal.
# Usage: scripts/run-stage7-circuit-canary.sh <circuit-recovery|rollback-claude> [--dry-run]
set -euo pipefail
...
CIRCUIT_WORKFLOW="$SB_HOME/projects/mixed-canary/WORKFLOW.circuit-recovery.md"
ROLLBACK_WORKFLOW="$SB_HOME/projects/mixed-canary/WORKFLOW.rollback-claude.md"
CHECKPOINT_DIR="$SB_HOME/projects/mixed-canary/stage7-checkpoints"
...
case "$PHASE" in
  circuit-recovery)
    ...
    ;;
  rollback-claude)
    TITLE="Stage 7 circuit checkpoint: Claude-only rollback"
    BODY_FILE="$CHECKPOINT_DIR/02-rollback-claude.md"
...
PROVISIONED_LABELS="$(gh_clean label list --repo "$REPO" --limit 100 \
  --json name --jq '.[].name')"
for label in status:drafting status:todo status:in-progress status:plan-review \
             status:human-review status:parked status:blocked \
             gate:triage-passed agent:claude agent:codex \
...
TEMP_DIR="$(mktemp -d "/private/tmp/switchboard-stage7-$PHASE.XXXXXX")"
...
[ -n "$STOP_STATUS" ] \
  || { tail -160 "$LOG" >&2 || true; fail "timed out after 45 minutes"; }
...
if [ "$PHASE" = "circuit-recovery" ]; then
  grep -q "provider circuit transition .*provider_id=codex .*circuit_state=open_cooldown .*failure_class=provider_unavailable .*cooldown_ms=300000" "$LOG" \
    || fail "log does not prove the typed cooldown open"
  grep -q "worker failed .*provider_id=codex .*failure_class=provider_unavailable .*retry_disposition=provider_wait" "$LOG" \
    || fail "log does not prove no-retry-burn provider wait"
  HALF_OPEN_COUNT="$(grep -c "provider circuit transition .*provider_id=codex .*circuit_state=half_open" "$LOG" || true)"
  [ "$HALF_OPEN_COUNT" = "1" ] \
    || fail "expected exactly one half-open transition, found $HALF_OPEN_COUNT"
  grep -q "provider circuit transition .*provider_id=codex .*circuit_state=closed" "$LOG" \
  ...
  SESSION_ONE_COUNT="$(grep "dispatched .*issue_identifier=$ISSUE_NUMBER .*provider_id=codex" "$LOG" \
    | grep -c "session_number=1" || true)"
  [ "$SESSION_ONE_COUNT" = "2" ] \
    || fail "provider outage consumed issue session accounting"
  ! grep -q "issue_identifier=$ISSUE_NUMBER .*status:parked\|ISSUE PARKED .*issue_identifier=$ISSUE_NUMBER" "$LOG" \
    || fail "provider outage parked the issue"
  TRANSCRIPTS="$(find "$WORKSPACE/.run/transcripts" -type f \
    -name 'codex-*.jsonl' 2>/dev/null | wc -l | tr -d '[:space:]')"
  [ "$TRANSCRIPTS" -ge 2 ] \
    || fail "circuit checkpoint did not retain outage and recovery transcripts"
fi
