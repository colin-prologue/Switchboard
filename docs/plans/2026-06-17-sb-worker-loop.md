...
> **Errata (2026-06-20, Codex C1):** the result-file instructions embedded below (`.switchboard/results/{T.id}.json`) are WRONG and were corrected in the live skill. The subagent's CWD is the worktree (no `.switchboard/`), and the engine reads `store.fname(id)` (slashes → `_`). The worker must compute an ABSOLUTE main-repo path with the id sanitized — `<repo>/.switchboard/results/<id-with-/-as-_>.json` — and pass it as `{result_path}`. See `.claude/skills/sb-work/` for the corrected protocols; do not copy the path text from this plan.
