...
providers:
  claude:
    ...
    # double quotes, so "Bash(./test.sh)" "Bash(./capture.sh:*)" can be substituted verbatim as
    # '"Bash(godot:*)"' without an escaping pass that sed would mangle.
    command: 'claude -p --model claude-opus-5 --verbose --output-format stream-json --permission-mode acceptEdits --allowedTools "Bash(git:*)" "Bash(gh:*)" "Bash(./test.sh)" "Bash(./capture.sh:*)"'
...
`colin-prologue/civ-life`. Your workspace is already a clean clone of that repo, checked out on
...
**Review bot for this project:** `chatgpt-codex-connector`
...
1. **Fetch its review of the exact sha you are reviewing.**
   ...
   ```
   gh pr view <pr> --repo colin-prologue/civ-life --json reviews,headRefOid
   ```
...
- **SHIP** — it runs, it matches intent, the `## In brief` block passes both
  ...
  ```
  gh pr merge <pr> --repo colin-prologue/civ-life --squash --delete-branch
  ```
...
```
Cross-model review: chatgpt-codex-connector reviewed <sha>, <n> finding(s), all resolved.
```
```
...
3. **Preflight — three checks before you open the PR.** These replace a triage
   ...
   - **It runs.** `./test.sh` exits 0. If the project has no verification
...
4. **Open the PR.**
   ...
   ```
   gh pr create --repo colin-prologue/civ-life --base main --title "<title>" --body-file <file>
   ```
...
expensive to reverse — write it to `.decisions/` as a short
